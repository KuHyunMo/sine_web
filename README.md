...
| **분류** | **기술 스택** | **선택 이유** |
| --- | --- | --- |
| **Frontend** | HTML5, CSS, JS | 웹 표준 기술<br>사용 경험이 있고 간단함 |
| **Backend** | **Python (Flask)** | 가벼우면서도 확장이 용이함 |
| **Database** | **MySQL** | 관계형 데이터베이스(RDBMS)의 표준<br>데이터 정규화 및 무결성 보장 |
| **Server** | **AWS EC2 (Ubuntu)** | 클라우드 인프라 경험<br>리눅스 환경에서의 서버 제어 경험<br>일정 기간동안 무료로 이용 가능 |
| **Proxy** | **Nginx** | 보안을 위한 리버스 프록시 구성 및 정적 파일 처리 최적화<br>AWS(Flask)를 외부망에 노출하지 않아 직접적인 공격 방지<br>요구조건에 있었음 |
